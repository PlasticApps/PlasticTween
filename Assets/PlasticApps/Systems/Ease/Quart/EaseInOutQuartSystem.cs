...
namespace PlasticApps.Systems.Ease
{
    ...
    public class EaseInOutQuartSystem : JobComponentSystem
    {
        [BurstCompile]
        [ExcludeComponent(typeof(TweenPaused), typeof(TweenComplete))]
        [RequireComponentTag(typeof(TweenEaseInOutQuart))]
        struct EaseJob : IJobForEach<TweenBase>
        {
            public void Execute(ref TweenBase tween)
            {
                unchecked
                {
                    ...
                    if (value < 1)
                    {
                        tween.Value = 0.5f * value * value * value * value;
                    }
                    else
                    {
                        value -= 2;
                        tween.Value = -0.5f * (value * value * value * value - 2);
                    }
                }
            }
        }
